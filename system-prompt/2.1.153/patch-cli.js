...
// Configuration
const EXPECTED_VERSION = '2.1.153';
const EXPECTED_HASHES = {
  npm: 'TODO',
  'native-linux-arm64': 'TODO',
  'native-linux-x64': 'TODO',
  'native-macos-arm64': '948042cc5aa11eb986bdccb540caa5134b65f698b22bfbb0a1f201baf46e4a73',
...
const patches = [
  // Banner stripping (must run before security-warning / url-warning, which now overlap)
  { name: 'Strip Pm3 banner: interactive-agent + URL/security IMPORTANTs (~496 bytes)',
    customRegex: /function ([a-zA-Z0-9_$]+)\(H\)\{return`\nYou are an interactive agent[\s\S]*?messages or local files\.`\}/,
    customReplace: 'function $1(H){return""}' },
  // Identity constants left intact — emptying them breaks Anthropic API cache_control scope contract (400 error)

  // Dynamic-section gutting (functions called via yG / array assembly in HZ()). Each returns null or "" so the caller's .filter(M=>M!==null) drops it.
  { name: 'Gut Zm3/IMO (# Doing tasks section, ~3953 bytes)',
    customRegex: /function ([a-zA-Z0-9_$]+)\(\)\{let _=\[\.\.\.\["Don't add features[\s\S]*?return\["# Doing tasks",\.\.\.[a-zA-Z0-9_$]+\([A-Za-z_]\)\]\.join\(`\n`\)\}/,
    customReplace: 'function $1(){return null}' },
  { name: 'Gut Vm3/mMO (# Session-specific guidance, ~3674 bytes)',
    customRegex: /function ([a-zA-Z0-9_$]+)\(H,_,q,K\)\{let O=[a-zA-Z0-9_$]+\(\),T=H\.has\([a-zA-Z0-9_$]+\)[\s\S]*?return\["# Session-specific guidance",\.\.\.[a-zA-Z0-9_$]+\([A-Za-z_]\)\]\.join\(`\n`\)\}/,
    customReplace: 'function $1(H,_,q,K){return null}' },
  { name: 'Gut wm3/VMO (# Text output / anti-verbosity, ~1744 bytes)',
    customRegex: /function ([a-zA-Z0-9_$]+)\(H\)\{if\(j\$\(H\)\)\{[\s\S]*?intermediate files\.`\}/,
    customReplace: 'function $1(H){return null}' },
  ...
  // git-commit-v2 and pr-creation-v2 dropped — banner patch now removes preamble, v1 alone is already lean
  { name: 'Slim Explore agent (~350 → 120 chars)', file: 'agent-explore' },
  // security-warning dropped — banner patch now strips entire intro block including this IMPORTANT line
  ...
  // url-warning dropped — banner patch now strips entire intro block including this IMPORTANT line
